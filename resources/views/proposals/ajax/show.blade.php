...
@if(!in_array('client', user_roles()))
    @if(!is_null($invoice->last_viewed))
        <x-alert type="info">
            {{$invoice->lead->client_name}} @lang('app.viewedOn') {{$invoice->last_viewed->timezone($settings->timezone)->format($settings->date_format)}}
            @lang('app.at') {{$invoice->last_viewed->timezone($settings->timezone)->format($settings->time_format)}}
