...
class AttendanceController extends AccountBaseController
{
    ...
    public function summaryData($request)
    {
        ...
        $this->weekMap = [
            Holiday::SUNDAY => __('app.su'),
            Holiday::MONDAY => __('app.mo'),
            Holiday::TUESDAY => __('app.tu'),
            Holiday::WEDNESDAY => __('app.we'),
            Holiday::THURSDAY => __('app.th'),
            Holiday::FRIDAY => __('app.fr'),
            Holiday::SATURDAY => __('app.sa'),
        ];
    ...
    public function show($id)
    {
        ...
        foreach ($attendanceActivity as $key => $activity) {
            ...
            $this->totalTime = $this->totalTime + $this->endTime->timezone($this->company->timezone)->diffInMinutes($activity->clock_in_time->timezone($this->company->timezone), true);
        }

        $this->maxClockIn = !($attendanceActivity->count() < $this->attendanceSettings->clockin_in_day) ? false : true;
