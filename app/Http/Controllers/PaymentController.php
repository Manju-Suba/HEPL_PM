...
class PaymentController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        if($this->viewBankAccountPermission == 'added'){
            $bankAccounts = $bankAccounts->where('added_by', user()->id);  /* @phpstan-ignore-line */
        }

        $bankAccounts = $bankAccounts->get();  /* @phpstan-ignore-line */
    ...
    public function update(UpdatePayments $request, $id)
    {
        ...
        return Reply::successWithData(__('messages.paymentSuccess'), ['redirectUrl' => route('payments.index')]);

    }

    public function show($id)
    {
        $this->payment = Payment::with(['invoice', 'project', 'currency', 'transactions' => function($q){
