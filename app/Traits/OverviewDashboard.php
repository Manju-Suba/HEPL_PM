    ...
    public function overviewDashboard()
    {
        ...
        $this->counts = DB::table('users')
            ->select(
                ...
                DB::raw('(select count(tasks.id) from `tasks` where (tasks.board_column_id != ' . $completedTaskColumn->id . ' and is_private = "0" AND tasks.company_id = '. company()->id .') and deleted_at IS NULL) as totalPendingTasks'),
