...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('due_date', function ($row) {
            ...
            if ($row->due_date->endOfDay()->isPast() && $row->board_column != "Completed") {
    ...
    public function query(Task $model)
    {
        ...
        $role=RoleUser::where('user_id',user()->id)->where('role_id','6')->select('role_id')->first();
        if($role != null)
        {

            $model = $model->leftJoin('projects', 'projects.id', '=', 'tasks.project_id')
            ->leftJoin('users as client', 'client.id', '=', 'projects.client_id')
            ->leftJoin('task_users as tu','tu.task_id','=','tasks.id')
            ->join('taskboard_columns', 'taskboard_columns.id', '=', 'tasks.board_column_id');
            $model->where(function ($query){
                $query->where('tasks.created_by',user()->id)->orwhere('task_users.user_id',user()->id)->orwhere('projects.project_admin',user()->id);
            });
        ...
        if ($request->type == 'public') {
            ...
            $model->where('tasks.board_column_id','!=', 4);
        }
